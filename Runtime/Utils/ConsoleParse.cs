	...
	internal static class ConsoleParse
	{
		public static CommandRequest Command(string input)
		{
			...
			var r = new CommandRequest
			{
				...
				optionalArgs = Array.Empty<(string, object)>(),
				type = ECommandType.MethodCall,
			};
			...
			if (input[i] == '=')
			{
				r.type = ECommandType.Assignment;
				i++;
			...
			else if (input[i] == '(')
			{
				if (input[^1] != ')')
				{
					throw new ConsoleException($"Expecting closing parentheses at {input.Length - 1}");
				}
				...
				if(input[i + 1] != ')')
				{
					...
					while (i < input.Length)
					{
						...
						if (input[i] == ',')
						{
							...
							if (input[i] == ')')
							{
								throw new ConsoleException($"Expecting more args {i}");
							}
						}
						i++;
					}
		...
		private static string ReadKey(ref int i, in string input)
		{
			...
			{
				throw new ConsoleException($"Keyword expected, got token '{input[i]}'");
			}
			var s = i;
			while (i < input.Length && Token.IsKeyToken(input[i])) { i++; }
			if (i == s)
			{
				throw new ConsoleException($"Invalid keyword at {s}, expected length > 0");
			}

			if (input[i - 1] == '.')
			{
				throw new ConsoleException($"Invalid keyword at {s}, invalid end token '.'");
			}
		...
		private static (string, object) ReadKeyedArg(ref int i, in string input)
		{
			if (i >= input.Length)
			{
				throw new ConsoleException($"Expected key at {i}");
			}
		...
		private static object ReadValue(ref int i, in string input)
		{
			...
			if (i >= input.Length)
			{
				throw new ConsoleException($"Expecting value at {i}, got EOL");
			}
			...
			else if (input[i] == '<')
			{
				...
				{
					throw new ConsoleException($"Invalid vector '{rawValue}'");
				}
				return vec;
			}

			throw new ConsoleException($"Unexpected input '{input[i]}' at {i}");
		}
		...
		private static object ReadColor(ref int i, in string input)
		{
			if((i + 1) >= input.Length)
			{
				throw new ConsoleException($"Expected color at '{i}', got end of input");
			}
			var s = i;
			...
			{
				throw new ConsoleException($"Invalid color string '{rawColor}'");
			}
		...
		private static object ReadNumeric(ref int i, in string input)
		{
			...
			while (i < input.Length && !Token.IsArgumentBreak(input[i]))
			{
				...
				{
					throw new ConsoleException($"Invalid num token at {i}");
				}
			...
			{
				throw new ConsoleException($"Expected number at {s}");
			}
		}
		...
		private static string ReadClosingToken(ref int i, in string input, in char token)
		{
			...
			if (e < 0)
			{
				throw new ConsoleException($"'{token}' expected, got end of input");
			}
			i++;
		...
		private static class Token
		{
			...
			public static bool IsBool(in char c) => c is 't' or 'f';
			public static bool IsSpace(in char c) => c is ' ' or '\t';
